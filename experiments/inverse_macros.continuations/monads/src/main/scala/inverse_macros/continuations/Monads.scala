...
object OptionMonad {
  ...
  implicit class OptionMReflect[X](val cx: Option[X]) extends AnyVal {
    @inline def reflect: X@cps[Option[Any]] = if (cx.isDefined) cx.get else shift((k: X => Option[Any]) => cx.flatMap(k))
  }
...
object StateMonad {
  ...
  @inline def get[S]: S@cps[S => (S, Any)] = shift((k: S => S => (S, Any)) => ((s: S) => (s, s)).flatMap(k))
  ...
  implicit class StateMReflect[S, X](val cx: S => (S, X)) extends AnyVal {
    @inline def reflect: X@cps[S => (S, Any)] = shift((k: X => S => (S, Any)) => cx.flatMap(k))
  }
...
object Monads {
  @inline def reify[X, CX](body: => X@cpsParam[CX, Any])(implicit munit: MUnit[X, CX]): CX = reset[CX, Any] {
    munit.unit(body)
  }.asInstanceOf[CX]
}